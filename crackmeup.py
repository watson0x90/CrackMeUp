...
def decode_hashcat_password(pw_string):
    """Decodes $HEX[...] encoded passwords from hashcat output."""
    if pw_string.startswith("$HEX[") and pw_string.endswith("]"):
        try:
            hex_part = pw_string[5:-1]
            return binascii.unhexlify(hex_part).decode('utf-8', errors='replace')
        except Exception:
            return pw_string
    return pw_string
...
EFFORT_LEVELS = {
    'low': {
        'wordlists': ["rockyou.txt", "InsideProFull", "2024_cracked.txt"],
        'rules': ["best64.rule", "hob064.rule", "yubaba64.rule", "ancestryCracked.rule"]
    },
    'medium': {
        'wordlists': [
            "rockyou.txt", "InsideProFull", "2024_cracked.txt", 
            "hk_hlm_founds.txt", "kaonashi.txt"
        ],
        'rules': [
            "best64.rule", "hob064.rule", "yubaba64.rule", "ancestryCracked.rule",
...
def generate_custom_rules(passwords, output_path):
    """Generates a Hashcat rule file based on common prefixes/suffixes in passwords."""
    if not passwords or len(passwords) < 5:
        return None
    ...
    for pw in passwords:
        ...
        # Extract 1-4 char prefixes/suffixes
        for i in range(1, 5):
            prefixes.append(pw[:i])
            suffixes.append(pw[-i:])

    # Find top patterns (occurring in at least 5% of passwords, or at least 3 times)
    min_occ = max(3, int(len(passwords) * 0.05))
    
    top_pre = pd.Series(prefixes).value_counts()
    top_pre = top_pre[top_pre >= min_occ].head(20).index.tolist()
    
    top_suf = pd.Series(suffixes).value_counts()
    top_suf = top_suf[top_suf >= min_occ].head(20).index.tolist()

    rules = [":", "c", "u", "l"] # Standard baseline: Nothing, Capitalize, Upper, Lower
    
    # Convert suffixes to append rules ($)
    for s in top_suf:
        rule = "".join([f"${c}" for c in s])
        if rule not in rules: rules.append(rule)
        
    # Convert prefixes to prepend rules (^)
    for p in top_pre:
        # Prepending "Adm" is ^m^d^A (reverse order)
        rule = "".join([f"^{c}" for c in p[::-1]])
        if rule not in rules: rules.append(rule)

    with open(output_path, 'w') as f:
        f.write("# Custom organizational rules generated by CrackMeUp\n")
        f.write("\n".join(rules))
        f.write("\n")
    
    return len(rules)

def create_csv_all_data(analyze_file, out_file, unhex=False):
    ...
    try:
        with open(analyze_file, "r") as f:
            for line in f:
                ...
                raw_password = temp[2]
                password = decode_hashcat_password(raw_password) if unhex else raw_password
...
def main():
    ...
    parser.add_argument('--auto-mask', action='store_true', help='Automatically run mask attacks for patterns found > threshold')
    parser.add_argument('--mask-threshold', type=int, default=20, help='Minimum occurrences to trigger an automated mask attack (default: 20)')
    parser.add_argument('--unhex', action='store_true', help='Decode $HEX[...] passwords in final reports and CSV')
    parser.add_argument('--report-only', action='store_true', help='Skip cracking and only generate reports from an existing session')
    parser.add_argument('--generate-rules', action='store_true', help='Generate a custom .rule file based on discovered patterns')
    parser.add_argument('--patd-format', action='store_true', help='Generate a user:hash:password file for "Password! At The Disco"')
    parser.add_argument('--rules-path', default=DEFAULT_RULES_PATH, help='Path to hashcat rules')
    parser.add_argument('--wordlist-path', default=DEFAULT_WORDLIST_PATH, help='Path to hashcat wordlists')
    ...
    generate_rules_flag = args.generate_rules
    patd_flag = args.patd_format

    # Normalize pot_file: strip .pot extension if user provided it to avoid .pot.pot
    if pot_file.lower().endswith('.pot'):
        pot_file = pot_file[:-4]
    ...
    try:
        if not report_only:
            ...
            print("\n[!] Skipping cracking phase (REPORT ONLY mode)")
    ...
    # Identify source potfiles: use session list, or fallback to the -p argument for standalone use
    source_pots = session["pot_file_list"]
    if not source_pots:
        # Normalize name for search: we expect a .pot extension on disk
        potential_pot = pot_file if pot_file.lower().endswith('.pot') else pot_file + '.pot'
        
        # Check relative to hash_dir or as absolute
        search_paths = [potential_pot, os.path.join(hash_dir, potential_pot)]
        
        for p in search_paths:
            if os.path.exists(p):
                source_pots = [p]
                print(f"[+] Using standalone potfile for reporting: {p}")
                break

    all_passwords = []
    temp_hash_list = []
    for pot in source_pots:
        if os.path.exists(pot):
            with open(pot, 'r') as infile:
                for line in infile:
                    ...
                    if len(temp) >= 2:
                        raw_pw = temp[1]
                        all_passwords.append(decode_hashcat_password(raw_pw) if unhex else raw_pw)
    ...
    if generate_rules_flag:
        custom_rule_path = os.path.join(hash_dir, "custom_organizational.rule")
        num_rules = generate_custom_rules(all_passwords, custom_rule_path)
        if num_rules:
            print(f"\n[+] Generated {num_rules} custom rules based on patterns in: {custom_rule_path}")
            print("    You can use this in your next run with: -r " + custom_rule_path)
    ...
    create_csv_all_data(hc_results_file, csv_name, unhex=unhex)
    print(f"Final report generated: {csv_name}")

    if patd_flag:
        patd_output_path = os.path.join(hash_dir, f"patd_import_{now.strftime('%Y-%m-%d-%H-%M-%S')}.txt")
        with open(hc_results_file, 'r') as infile, open(patd_output_path, 'w') as outfile:
            for line in infile:
                temp = line.strip().split(':')
                if len(temp) >= 3:
                    user = temp[0]
                    h = temp[1]
                    raw_pw = temp[2]
                    pw = decode_hashcat_password(raw_pw) if unhex else raw_pw
                    outfile.write(f"{user}:{h}:{pw}\n")
        print(f"[+] PATD format file generated: {patd_output_path}")
